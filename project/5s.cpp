...
int main() {
    const string conn_str = "dbname=decs user=postgres password=kali host=192.168.137.1";
    LibpqxxPool pool(50, conn_str);
    ...
    srv.Get("/val", [&](const Request &req, Response &res) {
        ...
        try {
            ...
            db::work txn{*conn};
            db::result r = txn.exec_params("SELECT value FROM kv_store WHERE id = $1", id_int);
            txn.commit();
            pool.release(conn);

            if (r.empty()) {
                res.status = 404;
                res.set_content("No value found for id: " + to_string(id_int), "text/plain");
            } else {
    ...
    srv.Post("/save", [&](const Request &req, Response &res) {
        ...
        try {
            db::connection *conn = pool.acquire();
            db::work txn{*conn};
            txn.exec_params("INSERT INTO kv_store (id, value) VALUES ($1, $2)", id_int, val);
            txn.commit();
            pool.release(conn);
    ...
    srv.Put("/put", [&](const Request &req, Response &res) {
        ...
        try {
            db::connection *conn = pool.acquire();
            db::work txn{*conn};
            db::result r = txn.exec_params("UPDATE kv_store SET value = $1 WHERE id = $2", val, id_int);
            txn.commit();
            pool.release(conn);
    ...
    srv.Delete("/delete", [&](const Request &req, Response &res) {
        ...
        try {
            db::connection *conn = pool.acquire();
            db::work txn{*conn};
            db::result r = txn.exec_params("DELETE FROM kv_store WHERE id = $1", id_int);
            txn.commit();
            pool.release(conn);
